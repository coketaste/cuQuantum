...
// Two-site shift-invert DMRG for a three-site transverse-field Ising model.
...
double denseMPSNorm(
    ...
    const std::array<int64_t, NUM_MPS_SITES - 1> & bondExtents)
{
  for (std::size_t bond = 0; bond < bondExtents.size(); ++bond) {
    if (bondExtents[bond] <= 0
        || bondExtents[bond] > MPS_BOND_EXTENTS[bond]) {
      std::fprintf(stderr, "Invalid current MPS bond extent\n");
      std::abort();
    }
  }
  ...
  for (int64_t p0 = 0; p0 < 2; ++p0) {
    for (int64_t p1 = 0; p1 < 2; ++p1) {
      for (int64_t p2 = 0; p2 < 2; ++p2) {
        ...
        for (int64_t b0 = 0; b0 < bondExtents[0]; ++b0) {
          for (int64_t b1 = 0; b1 < bondExtents[1]; ++b1) {
            amplitude +=
              tensors[0][p0 + 2 * b0]
...
struct NoisyPlusMPSStorage
{
  ...
  void build()
  {
    ...
    const double initialNorm =
      denseMPSNorm(hostTensors, MPS_BOND_EXTENTS);
...
constexpr int64_t SVD_MAX_EXTENT = 2;
constexpr double ZERO_CUTOFF = 0.0;
...
int main()
{
  ...
  std::cout
    << "Two-site shift-invert DMRG eigensolver example\n"
    ...
    << ", ABS/REL/DISCARDED_WEIGHT cutoffs=0\n";
  ...
  // Explicitly supply a lossless SVD policy for the bond-2 MPS.
  cudensitymatSVDConfig_t svdConfig = nullptr;
  HANDLE_CUDM_ERROR(cudensitymatCreateSVDConfig(handle, &svdConfig));
  HANDLE_CUDM_ERROR(cudensitymatSVDConfigSetAttribute(
    handle, svdConfig, CUDENSITYMAT_SVD_CONFIG_MAX_EXTENT,
    &SVD_MAX_EXTENT, sizeof(SVD_MAX_EXTENT)));
  HANDLE_CUDM_ERROR(cudensitymatSVDConfigSetAttribute(
    handle, svdConfig, CUDENSITYMAT_SVD_CONFIG_ABS_CUTOFF,
    &ZERO_CUTOFF, sizeof(ZERO_CUTOFF)));
  HANDLE_CUDM_ERROR(cudensitymatSVDConfigSetAttribute(
    handle, svdConfig, CUDENSITYMAT_SVD_CONFIG_REL_CUTOFF,
    &ZERO_CUTOFF, sizeof(ZERO_CUTOFF)));
  HANDLE_CUDM_ERROR(cudensitymatSVDConfigSetAttribute(
    handle, svdConfig,
    CUDENSITYMAT_SVD_CONFIG_DISCARDED_WEIGHT_CUTOFF,
    &ZERO_CUTOFF, sizeof(ZERO_CUTOFF)));
  HANDLE_CUDM_ERROR(
    cudensitymatEigenDecompositionScopeSplitDMRGConfigSetAttribute(
      handle, dmrgConfig,
      CUDENSITYMAT_EIGEN_SPLIT_SCOPE_DMRG_SVD_CONFIG,
  ...
  // Verify the returned MPS with an independent dense contraction over only
  // the current logical bond extents, using the maximum-buffer strides.
  std::array<int64_t, NUM_MPS_SITES - 1> currentBondExtents {};
  HANDLE_CUDM_ERROR(cudensitymatStateMPSGetCurrentBondExtents(
    handle, mpsState, currentBondExtents.data()));
  mpsStorage.download();
  const double representedMPSNorm =
    denseMPSNorm(mpsStorage.hostTensors, currentBondExtents);
