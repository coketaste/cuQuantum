...
// Two-site MPS-TDVP time propagation example.
//
// Demonstrates propagating a pure MPS state under a transverse-field Ising
// Hamiltonian encoded as an MPO, using the split-scope TDVP method with the
// TWO-site local update (CUDENSITYMAT_PROPAGATION_SPLIT_SCOPE_TDVP_NUM_SITES = 2)
// and Krylov subspace exponentiation.
//
// Unlike the single-site companion (mps_tdvp_example.cpp), the two-site sweep
// can dynamically grow the MPS bond dimensions: each two-site block is evolved
// and then re-split via a truncated SVD. This example therefore additionally:
//   * starts from small current bond extents (> 1, below the buffer maximum)
//     via cudensitymatStateMPSSetCurrentBondExtents;
//   * attaches a TDVP SVD configuration with a maximum-extent cap
//     (CUDENSITYMAT_SVD_CONFIG_MAX_EXTENT); and
//   * after each step queries cudensitymatStateMPSGetCurrentBondExtents and
//     compares against the maximum extents to monitor bond growth.
//
// Workflow:
...
//  3. Initialize input MPS to a polarized product state |000...0> and set small
//     current bond extents
//  4. Create TDVP time propagation object
//  5. Configure TDVP (num_sites = 2 + SVD config) / Krylov parameters
//  6. Prepare propagation and allocate workspace
//  7. Time-stepping loop; after each step monitor current vs. maximum extents
...
constexpr int64_t  MAX_BOND_DIM     = 8;    // maximum (buffer) bond dimension cap
constexpr int64_t  MPO_BOND_DIM     = 3;
constexpr int64_t  INITIAL_BOND_DIM = 2;    // small initial current bond dimension (> 1)
...
struct IsingMPO {
  ...
  void build() {
    ...
    for (int32_t site = 0; site < NUM_SITES; ++site) {
      ...
      // Library mode ordering (column-major): [left_bond, phys_ket, right_bond, phys_bra]
...
// ============================================================================
// Build a polarized product-state MPS  |000...0>  with given (maximum/buffer)
// bond extents
// ============================================================================
// Each MPS tensor A[site] has shape (bondL, phys, bondR) in column-major over
// the MAXIMUM (buffer) extents. For a product state only the (0,sigma,0) slice
// is nonzero; the smaller current bond extents (set separately) leave the rest
// of the buffer zero-padded.
...
struct ProductMPS {
  ...
  void build(const std::vector<int64_t>& maxBondDims) {
    const Complex zero{0.0, 0.0}, one{1.0, 0.0};
    ...
    for (int32_t site = 0; site < NUM_SITES; ++site) {
      const int64_t bL = (site == 0) ? 1 : maxBondDims[site - 1];
      const int64_t bR = (site == NUM_SITES - 1) ? 1 : maxBondDims[site];
      const int64_t vol = bL * PHYS_DIM * bR;
      hostTensors[site].assign(vol, zero);

      // T[aL, sigma, aR]  column-major over the maximum extents.
      auto idx = [&](int64_t aL, int64_t sigma, int64_t aR) -> int64_t {
        return aL + bL * (sigma + PHYS_DIM * aR);
      };

      hostTensors[site][idx(0, 0, 0)] = one;  // |0> on each site (product state)
...
void exampleWorkflow(cudensitymatHandle_t handle)
{
  ...
  // Maximum (buffer) bond extents, fixed at state creation: capped by the exact
  // Hilbert space dimension on each side and by MAX_BOND_DIM.
  std::vector<int64_t> maxBondDims(NUM_SITES - 1);
  for (int32_t i = 0; i < NUM_SITES - 1; ++i) {
    ...
    maxBondDims[i] = std::min({MAX_BOND_DIM, leftDim, rightDim});
  }

  // Initial current bond extents: > 1, clamped to the maximum, so the two-site
  // sweep starts below the buffer maximum and can grow the bonds.
  std::vector<int64_t> initialBondDims(NUM_SITES - 1);
  for (int32_t i = 0; i < NUM_SITES - 1; ++i)
    initialBondDims[i] = std::min<int64_t>(INITIAL_BOND_DIM, maxBondDims[i]);
  ...
  // Query storage sizes for each MPS tensor (sized for the MAXIMUM extents)
  ...
  // --- 4. Allocate GPU storage and initialise the product-state MPS for stateIn ---
  ProductMPS productMps;
  productMps.build(maxBondDims);
  ...
  HANDLE_CUDM_ERROR(cudensitymatStateAttachComponentStorage(handle,
                      ...
                      productMps.gpuPtrs.data(), componentSizes.data()));
  HANDLE_CUDM_ERROR(cudensitymatStateAttachComponentStorage(handle,
                      stateOut, numComponents,
                      stateOutBufs.data(), componentSizes.data()));

  // Record the small initial CURRENT (valid) bond extents (metadata only).
  HANDLE_CUDM_ERROR(cudensitymatStateMPSSetCurrentBondExtents(handle,
                      stateIn, initialBondDims.data()));
  HANDLE_CUDM_ERROR(cudensitymatStateMPSSetCurrentBondExtents(handle,
                      stateOut, initialBondDims.data()));

  if (verbose) {
    std::cout << "Initialized MPS states (polarized product state |000...0>)\n";
    std::cout << "Maximum (buffer) bond extents:";
    for (auto b : maxBondDims) std::cout << " " << b;
    std::cout << "\nInitial current bond extents:";
    for (auto b : initialBondDims) std::cout << " " << b;
    std::cout << "\n";
  }
  ...
  // TDVP config: 2nd-order, TWO-site, with an attached SVD truncation policy.
  ...
  HANDLE_CUDM_ERROR(cudensitymatCreateTimePropagationScopeSplitTDVPConfig(handle, &tdvpConfig));
  {
    ...
    HANDLE_CUDM_ERROR(cudensitymatTimePropagationScopeSplitTDVPConfigSetAttribute(handle,
                        ...
                        &order, sizeof(order)));
    const int32_t numSites = 2;
    HANDLE_CUDM_ERROR(cudensitymatTimePropagationScopeSplitTDVPConfigSetAttribute(handle,
                        tdvpConfig,
                        CUDENSITYMAT_PROPAGATION_SPLIT_SCOPE_TDVP_NUM_SITES,
                        &numSites, sizeof(numSites)));
  }

  // SVD truncation policy for the two-site splits (captured by value at Configure
  // time, so it can be destroyed right after). Here just a global bond cap.
  cudensitymatSVDConfig_t svdConfig;
  HANDLE_CUDM_ERROR(cudensitymatCreateSVDConfig(handle, &svdConfig));
  {
    const int64_t svdMaxExtent = MAX_BOND_DIM;
    HANDLE_CUDM_ERROR(cudensitymatSVDConfigSetAttribute(handle, svdConfig,
                        CUDENSITYMAT_SVD_CONFIG_MAX_EXTENT,
                        &svdMaxExtent, sizeof(svdMaxExtent)));
  }
  HANDLE_CUDM_ERROR(cudensitymatTimePropagationScopeSplitTDVPConfigSetAttribute(handle,
                      tdvpConfig,
                      CUDENSITYMAT_PROPAGATION_SPLIT_SCOPE_TDVP_SVD_CONFIG,
                      &svdConfig, sizeof(svdConfig)));
  ...
  HANDLE_CUDM_ERROR(cudensitymatDestroySVDConfig(svdConfig));
  ...
  if (verbose)
    std::cout << "Configured TDVP (order=2, num_sites=2, SVD max_extent=" << MAX_BOND_DIM
              << ") with Krylov (max_dim=10, tol=1e-8)\n";
  ...
  std::vector<int64_t> currentBonds(numBonds, 0);
  ...
  for (int32_t step = 0; step < NUM_STEPS; ++step) {
    ...
    // Compare the evolved state's current bond extents against the maximum
    // (buffer) extents to see how the bonds grow / get truncated.
    HANDLE_CUDM_ERROR(cudensitymatStateMPSGetCurrentBondExtents(handle,
                        stateOut, currentBonds.data()));
    if (verbose) {
      std::cout << "Step " << (step + 1) << "/" << NUM_STEPS
                << "  current/maximum bond extents:";
      bool anyTruncated = false;
      for (int32_t b = 0; b < numBonds; ++b) {
        std::cout << " " << currentBonds[b] << "/" << maxBondDims[b];
        if (currentBonds[b] < maxBondDims[b]) anyTruncated = true;
      }
      std::cout << (anyTruncated ? "  (some bonds below maximum)" : "  (all bonds at maximum)") << "\n";
    }
