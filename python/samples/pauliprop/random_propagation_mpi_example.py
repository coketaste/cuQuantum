...
"""
Example: distributed Pauli propagation over a large random expansion (MPI provider).
...
Requirements:
    - mpi4py
    - CUPAULIPROP_COMM_LIB environment variable pointing to the MPI
...
.. code-block:: bash
   ...
   export CUPAULIPROP_COMM_LIB=<path>/libcupauliprop_distributed_interface_mpi.so
   mpirun -n <num_ranks> python3 random_propagation_mpi_example.py
"""
...
def main():
    if "mpi" not in os.path.basename(os.environ.get("CUPAULIPROP_COMM_LIB", "")).lower():
        raise RuntimeError(
            "Set CUPAULIPROP_COMM_LIB to the MPI distributed-interface library."
        )
    ...
    with cp.cuda.Device(device_id):
        ...
        handle.set_communicator(mpi_comm, provider="MPI")
        propagate(handle, mpi_comm, provider_label="MPI")
